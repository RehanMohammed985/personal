      ...
      <div className="sm:hidden fixed bottom-4 left-4 z-40 flex flex-row space-x-3 bg-black/80 p-2 rounded-xl border border-gray-800">
        {leftSideSocialLinks.map((social, index) => (
          <motion.a
            ...
            transition={{ duration: 0.6, delay: 0.7 + index * 0.1 }}
            whileHover={{ scale: 1.1, x: 5 }}
            className="w-10 h-10 bg-white rounded-xl flex items-center justify-center hover:bg-gray-100 transition-all duration-200 group"
          >
            <social.icon className="w-5 h-5 text-black group-hover:scale-110 transition-transform" />
          </motion.a>
        ))}
      </div>
